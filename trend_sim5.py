...
def simulate_price(current_price, monthly_growth, time_index, trend_direction, ath, atl):
    ...
    # Ensure price does not go below ATL
    if new_price < atl:
        new_price = atl + np.random.normal(0, noise_level * atl)
    return new_price
...
last_atr = df['price'].rolling(window=14).apply(lambda x: np.max(x) - np.min(x)).iloc[-1] / 14
initial_ath = df['price'].max()
initial_atl = df['price'].min()
...
for i, date in enumerate(dates_until_2050):
    ...
    predicted_price = simulate_price(last_average_price, monthly_growth_realistic, i, trend_direction, ath, atl)
    optimistic_price = simulate_price(last_average_price, monthly_growth_optimistic, i, trend_direction, ath, atl)
    pessimistic_price = simulate_price(last_average_price, monthly_growth_pessimistic, i, trend_direction, ath, atl)
    ...
    if i >= short_window:
        short_mavg = pd.Series(predicted_prices[-short_window:]).mean()
    if i >= long_window:
        long_mavg = pd.Series(predicted_prices[-long_window:]).mean()
    ...
    if short_mavg > long_mavg:
        ...
        trend_direction = 'bearish'
         ...
         # Update ATR dynamically
    if len(predicted_prices) > 14:
        last_atr = pd.Series(predicted_prices[-14:]).apply(lambda x: np.max(x) - np.min(x)).mean() / 14

    if i % (2 * 12) == 0:  # Every 24 months
        ath *= 1.5
        print(ath)
